# Artemis v12 — v11 plus application logging to stdout (structured logs +
# gunicorn access/error logs). Multi-stage: build deps stay in the builder; the
# final image ships only a virtualenv + app, as a non-root user, pinned by digest.
...
# --access-logfile/-error-logfile "-" send gunicorn's request + error logs to stdout,
# alongside the app's own logging, so `docker logs` shows everything.
CMD ["gunicorn", "--bind", "0.0.0.0:5000", "--workers", "1", \
     "--access-logfile", "-", "--error-logfile", "-", "artemis:app"]
